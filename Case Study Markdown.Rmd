...
vix_dates <- fread("E:\\Documents\\dates.csv")
...
colnames(vix_results) <- c('date', 'close')

i <- 1

while(i < 13){
  sp500_near <- filter(sp500, Time == dates[i] & days_to_expiry == vix_dates[i,3])
  sp500_next <- filter(sp500, Time == dates[i] & days_to_expiry == vix_dates[i,4])
